...
## Index rebuilds
To rebuild the index we can refresh it with the domain-events flag set to false to suppress generating events when diffs are found.

```shell
http PUT /refresh-index?domain-events=false
```
...
If the index build fails - there are messages left on the index dead letter queue (DLQ) - then it may take user intervention to clear the DLQ if some messages are genuinely
