...
# Check that we can connect to open search
if ! OUTPUT=$(http --check-status --ignore-stdin "$OPENSEARCH_PROXY_URL"); then
  echo -e "\nUnable to talk to open search API"
  echo "$OUTPUT"
  exit 1
fi

# Grab last restore date from open search
SAVED_RESTORE_DATE=$(check_http GET "$OPENSEARCH_PROXY_URL/restore-status/_doc/1" | jq -r '._source.date')
...
check_http --print=Hbh DELETE "$OPENSEARCH_PROXY_URL/$INDICES"
sleep 10

# so have to then wait until we have removed the correct amount of indices
# can't just GET on our indices since open search will return 404 if ANY of them exist, whereas we need to check ALL
echo -e "\nWaiting for indices to be removed"
COUNT=0
while [[ $(check_http GET "$OPENSEARCH_PROXY_URL/_cat/indices?format=json" | jq length) -ne $EXPECTED_INDEX_COUNT ]]; do
  echo -n "."
  COUNT=$((COUNT+1))
  if [[ $COUNT -gt 20 ]]; then
...
echo -e "\nRestore successful"
check_http --print=b GET "$OPENSEARCH_PROXY_URL/_cat/indices/$INDICES"

# now stash away the restore status in an open search index
check_http --print=Hbh POST "$OPENSEARCH_PROXY_URL/restore-status/_doc/1" "date=$DATABASE_RESTORE_DATE"
