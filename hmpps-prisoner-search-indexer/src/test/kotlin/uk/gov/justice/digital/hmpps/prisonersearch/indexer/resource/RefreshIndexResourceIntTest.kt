...
class RefreshIndexResourceIntTest : IntegrationTestBase() {
  ...
  @MockitoSpyBean
  private lateinit var prisonerSynchroniserService: PrisonerSynchroniserService
  ...
  fun setUp() {
    ...
    prisoners.forEach { indexPrisoner(it.prisonerNumber) }
    await untilCallTo { prisonerRepository.count() } matches { it == 2L }
    // need to purge 3 events generated by indexing the prisoners
    await untilCallTo { getNumberOfMessagesCurrentlyOnDomainQueue() } matches { it == 3 }
