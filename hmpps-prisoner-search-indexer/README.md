...
This service maintains two indexes `prisoner-search-green` and `prisoner-search-blue` know in the code as `GREEN` and `BLUE`.
...
Once the index has finished, if there are no errors then the (housekeeping cronjob)[#housekeeping-cronjob] will mark the
index as complete and switch to the new index.
...
Given the state of each index is itself held in OpenSearch under the `in-progress` index with a single "document" 
when the INDEX_A/INDEX_B indexes switch there are actually two changes:
* The document in `offender-index-status` to indicate which index is currently active
* The ES `current-index` is switched to point at the active index. This means external clients can safely use the 
* `offender` index without any knowledge of the INDEX_A/INDEX_B indexes.
