...
class IntMatcherTest {
  ...
  fun `should not allow missing min and max values`() {
    val matcher = IntMatcher("heightCentimetres")
  ...
  fun `should not allow max less than min`() {
    val matcher = IntMatcher("heightCentimetres", minValue = 150, maxValue = 149)
  ...
  fun `should allow min equal to max if both inclusive`() {
    val matcher = IntMatcher("heightCentimetres", minValue = 150, minInclusive = true, maxValue = 150, maxInclusive = true)
  ...
  fun `should not allow max less than min when exclusive`() {
    ...
    val matcher = IntMatcher("heightCentimetres", minValue = 150, minInclusive = false, maxValue = 151, maxInclusive = false)
