...
class PrisonerSearchResourceTest : AbstractSearchDataIntegrationTest() {
  @Nested
  @DisplayName("POST /prisoner-search/match-prisoners")
  inner class MatchPrisoners {
    @Test
    ...
    fun `access forbidden when no role`() {
      webTestClient.post().uri("/prisoner-search/match-prisoners")
        .bodyValue(SearchCriteria("A7089EY", "john", "smith", listOf("MDI")))
    ...
    fun `bad request when no criteria provided`() {
      webTestClient.post().uri("/prisoner-search/match-prisoners")
        .bodyValue(SearchCriteria(null, null, null))
  ...
  @Nested
  @DisplayName("POST /prisoner-search/prison/{prisonId}")
  inner class FindByPrison {
    ...
    inner class Pagination {
      ...
      fun `will default the page size if empty parameter provided`() {
        prisonSearch(queryParams = "?size=")
          .expectBody().jsonPath("pageable.pageSize").isEqualTo("10")
      }

      @Test
      fun `will default the page size if invalid size provided`() {
        prisonSearch(queryParams = "?size=0&page=1")
          .expectBody().jsonPath("pageable.pageSize").isEqualTo("10")
      }
      ...
      fun `will default the page number if empty parameter provided`() {
        prisonSearch(queryParams = "?page=")
          .expectBody().jsonPath("pageable.pageNumber").isEqualTo("0")
      }

      @Test
      fun `will default the page number if invalid value provided`() {
        prisonSearch(queryParams = "?size=1&page=-1")
          .expectBody().jsonPath("pageable.pageNumber").isEqualTo("0")
      }
    }
